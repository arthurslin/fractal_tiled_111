...
void consumer_funct(FractalMessageQueue *queue)
{
    while (true)
    {
...
int main()
{
    ...
    for (size_t i = 0; i < num_tiles_x; i++)
    {
        ...
        for (size_t j = 0; j < num_tiles_y; j++)
        {
            ...
            std::shared_ptr<FractalMessage> msg = std::make_shared<FractalMessage>(&img, fractal_tile_min_x, fractal_tile_min_y, fractal_tile_max_x, fractal_tile_max_y, img_tile_min_x, img_tile_min_y, img_tile_max_x, img_tile_max_y, img_size_x, img_size_y);
    ...
    {
        std::shared_ptr<std::thread> thread = std::make_shared<std::thread>(consumer_funct, &queue);
